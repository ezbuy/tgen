...
func (o *SwiftGen) Generate(tplpath string, parsedThrift map[string]*parser.Thrift) ([]langs.GenResult, error) {
	...
	t, err := template.New(tplname).Funcs(funcMap).ParseFiles(tplpath)
	if err != nil {
		return nil, err
	}
	...
	for k, v := range parsedThrift {
		...
		results = append(results, langs.GenResult{Filename: name + EXT, Data: buf.Bytes()})
	}
